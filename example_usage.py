...
def run_classification_example():
    ...
    try:
        ...
        # FIX 1: Standardize class labels to title case
        train_data['class'] = train_data['class'].str.title()

        # Identify NDVI columns (ends with '_N')
        ndvi_cols = [col for col in train_data.columns if col.endswith('_N')]

        # FIX 2: Scale NDVI values by dividing by 10,000
        train_data[ndvi_cols] = train_data[ndvi_cols] / 10000.0
        ...
        # FIX: Only scale NDVI values (test data has no 'class' column)
        test_data[ndvi_cols] = test_data[ndvi_cols] / 10000.0
